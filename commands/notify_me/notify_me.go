...
func (c *Command) Run(api *tgbotapi.BotAPI, update *tgbotapi.Update) {
	...
	notifier.Get().Add(user.Data[0].ID, fmt.Sprintf("@%s", userName))

	msg := tgbotapi.NewMessage(update.Message.Chat.ID, fmt.Sprintf("добавил тебя в оповещения для %s", login))
	_, _ = api.Send(msg)
}
